...
class HolidaySlzController {
	async index(request: Request , response: Response , next: NextFunction) {
		try {
			const holidays = await knex<HolidaySlzRecord>("holidays_slz")
	...
	async create(request: Request , response: Response , next: NextFunction){
		try {
			...
			await knex<HolidaySlzRecord>("holidays_slz").insert({ holiday_name , day ,  month , optional })
	...
	async delete(request: Request , response: Response , next: NextFunction){
		try {
			...
			const holiday = await knex<HolidaySlzRecord>("holidays_slz")
				.select()
				.where({ id })
				.first()
