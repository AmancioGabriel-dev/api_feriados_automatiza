...
class HolidayTheController {
	async index(request: Request , response: Response , next: NextFunction) {
		try {
			const holidays = await knex<HolidayTheRecord>("holidays_the")
	...
	async create(request: Request , response: Response , next: NextFunction){
		try {
			...
			await knex<HolidayTheRecord>("holidays_the").insert({ holiday_name , day ,  month , optional })
	...
	async delete(request: Request , response: Response , next: NextFunction){
		try {
			...
			const holiday = await knex<HolidayTheRecord>("holidays_the")
				.select()
				.where({ id })
				.first()
			...
			await knex<HolidayTheRecord>("holidays_the").delete().where({ id })
